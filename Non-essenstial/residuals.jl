...
function ipm_obj(x_a, μ, problem::SEQUOIA_pb)
    ...
    return sum( (problem.gradient(x)+jac'*λ).^2 )+ sum(constraint_val.^2) + norm(2*ν.*s.^2 .-μ)^2 + sum((max.(0.0, -ν) ).^2 )
end
...
function ipm_obj(x_a, μ, problem::CUTEstModel)
    ...
    return norm(grad(problem,x)+Jac'*λ)^2 + sum(cons.^2) + norm(2*ν.*s.^2 .-μ)^2 + sum((max.(0.0, -ν) ).^2 )
end
...
function ipm_grad!(g, x_a, μ, problem::CUTEstModel)
    ...
    g[problem.meta.nvar+eq+1:problem.meta.nvar+length(λ)] .= g[problem.meta.nvar+eq+1:problem.meta.nvar+length(λ)] .+ 4*(2*ν.*s.^2 .- μ).*(s.^2) .- 2.0*max.(0.0, -ν) 
