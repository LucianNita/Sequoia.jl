export qpm_solve_lite
# Quadratic Penalty Method (QPM) Implementation using Optim.jl
function qpm_solve_lite!(problem::SEQUOIA_pb, inner_solver, options, time, x, previous_fval, iteration)
    ...
    while iteration < problem.solver_settings.max_iter_outer && time < problem.solver_settings.max_time_outer
        ...
        if conv
            solver_status = :first_order # Solver status

            step = SEQUOIA_Solution_step(iteration, abs(fval - previous_fval), solver_status, result.time_run, result.iterations, x, fval, problem.gradient(x), problem.constraints(x), [penalty_param])
