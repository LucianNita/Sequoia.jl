...
Solves a constrained optimization problem using the Quadratic Penalty Method (QPM) with `Optim.jl` for solving subproblems. Tracks and returns the solution history throughout the iterations.
...
- `solution_history`: A `SEQUOIA_Iterates` object containing the history of `SEQUOIA_Solution_step` objects for each iteration.
    - Each `SEQUOIA_Solution_step` includes:
        ...
        - `outer_iteration_number`: Iteration number of the outer QPM loop.
