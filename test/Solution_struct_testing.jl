...
@testset "SEQUOIA_Solution_step constructors and validation" begin
    ...
    step = SEQUOIA_Solution_step([1.0, 2.0], 0.5, [0.1, 0.2], nothing, 0.01, 1e-6, 10, 5, 0.02, :success)
    @test step.x == [1.0, 2.0]
    @test step.fval == 0.5
...
@testset "SEQUOIA_Iterates, add_step!, and getter functions" begin
    ...
    # Add a step
    step1 = SEQUOIA_Solution_step([1.0, 2.0], 0.5, [0.1, 0.2], nothing, 0.01, 1e-6, 10, 5, 0.02, :success)
    add_step!(iterates, step1)
    @test length(iterates.steps) == 1
    @test iterates.steps[1] == step1

    # Add another step
    step2 = SEQUOIA_Solution_step([1.1, 2.1], 0.45, [0.05, 0.15], [0.1, 0.2], 0.02, 1e-7, 11, 6, 0.03, :failed)
    ...
    empty_iterates = SEQUOIA_Iterates{Float64}()
