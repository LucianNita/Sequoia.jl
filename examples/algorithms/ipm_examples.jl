...
#function example_simple_equality()
    # Objective: Minimize f(x) = (x1 - 2)^2 + (x2 - 3)^2
    objective_fn = x -> (x[1] - 1.0)^2 + (x[2] - 3.0)^2

    # Constraint: x1 + x2 - 5 = 0
    constraints_fn = x -> [x[1] + x[2] - 3.0, x[1] - 1.0, x[2] - 4.0]

    # Gradient of the objective
    gradient_fn = x -> [2.0 * (x[1] - 1.0), 2.0 * (x[2] - 3.0)]

    # Jacobian of the constraint
    jacobian_fn = x -> [1.0 1.0; 1.0 0.0; 0.0 1.0] 
    ...
    pb = SEQUOIA_pb(
        ...
        jacobian = jacobian_fn,               # Jacobian of the constraints
        eqcon = [1],                          # Equality constraint index
        ineqcon = [2,3],                         # No inequality constraints
        solver_settings = SEQUOIA_Settings(:IntPt, :LBFGS, false, 10^-6, 1000, 300, 10^-6),
    )
    ...
    for step in pb.solution_history.iterates
        ...
        println("Solution: ", step.x)
        println("Objective Value: ", step.fval)
        println("Constraint Violation: ", step.cval)
    end
